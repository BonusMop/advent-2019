import { DayThreeB } from './DayThreeB';
import { MockInput } from '../Input/MockInput';

test('calculates shortest path', async () => {
    const puzzle: DayThreeB = new DayThreeB(new MockInput([]));

    const firstMap = puzzle.mapLocations('R8,U5,L5,D3', true);
    puzzle.mapLocations('U7,R6,D4,L4', false, firstMap);
    expect(puzzle.nearestIntersection(firstMap)).toBe(30);
    ...
    expect(puzzle.nearestIntersection(secondMap)).toBe(610);
    ...
    expect(puzzle.nearestIntersection(thirdMap)).toBe(410);
});
